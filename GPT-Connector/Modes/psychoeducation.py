...
def psychoeducation(inputType,iprompt, button):
    iprompt=[
        {"role": "system", "content": f"You are an AI psychoeducation teacher to a 9 year old who wants to learn about psychology. Conversation history: {iprompt}"}, #In order to properly train it I need to be able to signal where a conversation ends and where one begins. I should probably look into this later
        #add psychoeducation
    ...
    psychoeducationTools = tools[:len(tools)-2]
    #generate first message
    iprompt,text,functionCalled=prepare_message(iprompt,2,psychoeducation)
